...
function Riven:Combo()
	...
	local atarg = _G.SDK.TargetSelector:GetTarget(125)
...
function Riven:JungleClear()
	...
	local minion = Game.Minion(i)
		if  minion.team == 300 or 200 then
			...
			if self:IsValidTarget(minion,120) and myHero.pos:DistanceTo(minion.pos) < 120 and myHero.attackData.state == STATE_WINDDOWN then
			Control.Move()
			end
		end
	end
