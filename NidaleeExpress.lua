...
function Nidalee:LoadSpells()
	...
	W = {Range = 900, width = 40, Delay = 0.85, Radius = 40, Speed = 1450, Collision = false, aoe = true}
	E = {Range = 600, Delay = 0.25}
    QC = {Range = 200, Delay = 0.25}
	WC = {Range = 700, Delay = 0.25}
	EC = {Range = 1050, Delay = 0.25}

end

function Nidalee:LoadMenu()
	self.Menu = MenuElement({type = MENU, id = "NidaleeExpress", name = Scriptname})
...
function Nidalee:HasBuff(unit, buffname)
...
function ForceCat()
    local RRTarget = CurrentTarget(1000)
	local count = 0
...
function Nidalee:Jungle()
    ...
    if minion and minion.team == 300 or minion.team ~= myHero.team then
		...
		if self:CanCast(_R) then
        	if self.Menu.ClearMode.UseR:Value() and minion and myHero:GetSpellData(_Q).name == "JavelinToss" then
            	if myHero.pos:DistanceTo(minion.pos) < 800 and not self:CanCast(_Q) and not self:CanCast(_W) then
    	...
    	if self:CanCast(_R) then 
        	if self.Menu.ClearMode.UseR:Value() and minion and myHero:GetSpellData(_E).name == "Swipe" then
            	if not self:CanCast(_Q) and not self:CanCast(_E) and not self:CanCast(_W) then
					if Game.Timer() - LastR > 8 then
			    		Control.CastSpell(HK_R)
					end
