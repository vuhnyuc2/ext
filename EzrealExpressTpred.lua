...
class "Ezreal"
local Scriptname,Version,Author,LVersion = "EzrealExpressTpred","v1.0","Tocsin","7.17"
...
function Ezreal:Draw()
    if self:CanCast(_Q) then Draw.Circle(myHero.pos, 1100, 3,  Draw.Color(255, 255, 000, 255)) end
    ...
    if self:CanCast(_Q) then
			local temptarget = CurrentTarget(Q.Range)
			if temptarget == nil then return end
			local temppred
			local collisionc = Q.ignorecol and 0 or Q.minionCollisionWidth
			
			if (TPred) then
				local castpos,HitChance, pos = TPred:GetBestCastPosition(temptarget, Q.Delay , Q.Width, Q.Range,Q.Speed, myHero.pos, not Q.ignorecol, Q.Type )
...
function Ezreal:Combo()
    local temptarget = CurrentTarget(R.Range)
    if temptarget == nil then return end
    if self.Menu.ComboMode.UseQ:Value() and temptarget and self:CanCast(_Q) then
	...
	if self.Menu.ComboMode.UseW:Value() and temptarget and self:CanCast(_W) then
		if self:EnemyInRange(W.Range) then 
			local castpos,HitChance, pos = TPred:GetBestCastPosition(temptarget, W.Delay , W.Width, W.Range,W.Speed, myHero.pos, W.ignorecol, W.Type )
    ...
    if self:EnemyInRange(500) and not self:CanCast(_Q) then 
        if temptarget then
            if myHero.pos:DistanceTo(temptarget.pos) < 500 then
			    UseBotrk()
            end
        end
    end
 
	if self.Menu.ComboMode.UseE:Value() and temptarget and self:CanCast(_E) then
		if myHero.pos:DistanceTo(temptarget.pos) > 850 then
			Control.CastSpell(HK_E, temptarget)
		else
			local c1, c2, r1, r2 = Vector(myHero.pos), Vector(temptarget.pos), myHero.range, 525 
	...
	if self.Menu.ComboMode.UseR:Value() and temptarget then
		local RDamage = (self:CanCast(_R) and getdmg("R",temptarget,myHero) or 0)
		if RDamage > temptarget.health and myHero.pos:DistanceTo(temptarget.pos) > 650 then
			local castpos,HitChance, pos = TPred:GetBestCastPosition(temptarget, R.Delay , R.Width, R.Range,R.Speed, myHero.pos, R.ignorecol, R.Type )
		    if (HitChance > 0 ) and self:CanCast(_R) then
...
function Ezreal:Harass()
    local temptarget = CurrentTarget(Q.Range)
	if temptarget == nil then return end
	if self.Menu.HarassMode.UseQ:Value() and temptarget and self:CanCast(_Q) then
		local castpos,HitChance, pos = TPred:GetBestCastPosition(temptarget, Q.Delay , Q.Width, Q.Range,Q.Speed, myHero.pos, not Q.ignorecol, Q.Type )
