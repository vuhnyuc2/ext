...
local Qcollision = Collision:SetSpell(1000, 1500, .25, 75, true)
local qSpellData = {speed = 1500, delay = 0.25, range = 1000}
    ...
    local target = _G.SDK.TargetSelector:GetTarget(qSpellData.range)
        if target then
			...
			local pred = qSpell:GetPrediction(target, myHero.pos)
				if pred and pred.hitChance >= 0.25 and pred:mCollision() == 0 then
