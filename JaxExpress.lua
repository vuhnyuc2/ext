...
class "Jax"
local Scriptname,Version,Author,LVersion = "JaxExpress","v1.0","Tocsin","7.17"
...
function Jax:LoadMenu()
	self.Menu = MenuElement({type = MENU, id = "JaxExpress", name = Scriptname})
...
function Jax:Draw()
    ...
    if self:CanCast(_Q) then Draw.Circle(myHero.pos, 700, 3,  Draw.Color(255, 000, 222, 255)) end
	...
	if self.Menu.ComboMode.Key:Value() then
		Draw.Text("Insta E: On", 20, textPos.x - 33, textPos.y + 50, Draw.Color(255, 000, 255, 000)) --above built in smite msg incase jungle jax
	else
...
function Jax:Combo()
    if self.Menu.ComboMode.UseHYDRA:Value() and not self:HasBuff(myHero, "Empower") and self:EnemyInRange(124) then
...
----------------------------JaxCounterStrike
    if self:CanCast(_E) and self:EnemyInRange(720) then 
		...
		if self.Menu.ComboMode.UseE:Value() and self.Menu.ComboMode.Key:Value() == false and ETarget and not self:HasBuff(myHero, "JaxCounterStrike") then
			if self:EnemyInRange(125) and myHero.pos:DistanceTo(ETarget.pos) < 130 then
				Control.CastSpell(HK_E)
			end
		end
        if self.Menu.ComboMode.UseE:Value() and self.Menu.ComboMode.Key:Value() == true and ETarget and not self:HasBuff(myHero, "JaxCounterStrike") then
	...
	if self:CanCast(_Q) and self:EnemyInRange(695) then 
		local QTarget = CurrentTarget(695)
		if self.Menu.ComboMode.UseQ:Value() and QTarget then
            if self:EnemyInRange(695) and myHero.pos:DistanceTo(QTarget.pos) > 250 then
    ...
    if self:CanCast(_R) and self:EnemyInRange(700) then 
        local RTarget = CurrentTarget(700)
        if self.Menu.ComboMode.UseR:Value() and RTarget then
            if myHero.health/myHero.maxHealth < .60 and myHero.pos:DistanceTo(RTarget.pos) < 250 then
...
function Jax:EnemyInRange(range)
	local count = 0
...
function Jax:Harass()
    ...
    if self:CanCast(_E) and self:EnemyInRange(720) then 
		local ETarget = CurrentTarget(720)
		if self.Menu.HarassMode.UseE:Value() and self.Menu.ComboMode.Key:Value() == false and ETarget and not self:HasBuff(myHero, "JaxCounterStrike") then
			if self:EnemyInRange(125) and myHero.pos:DistanceTo(ETarget.pos) < 130 then
				Control.CastSpell(HK_E)
			end
		end
        if self.Menu.HarassMode.UseE:Value() and self.Menu.ComboMode.Key:Value() == true and ETarget and not self:HasBuff(myHero, "JaxCounterStrike") then
			if self:EnemyInRange(720) and self:CanCast(_Q) and myHero.pos:DistanceTo(ETarget.pos) < 720 then
				Control.CastSpell(HK_E)
			end
		end
...
function Jax:Jungle()
    ...
    if self.Menu.ComboMode.UseHYDRA:Value() and not self:HasBuff(myHero, "Empower") and minion then
