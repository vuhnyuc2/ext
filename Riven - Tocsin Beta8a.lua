...
function Riven:Combo()
    ...
    if self.Menu.Combo.UseW:Value() and self:CanCast(_W) and myHero.levelData.lvl <= 2 then --for lvl 1-2
		self:CastW(wtarg)
	end

    if self.Menu.Combo.UseQ:Value() and self:CanCast(_Q) and myHero.levelData.lvl <= 2 then --for lvl 1 mainly
    	self:LowQ(ltarg)
    end
...
function Riven:Harass() 
	...
	if self.Menu.Combo.UseQ:Value() and self:CanCast(_Q) and myHero.levelData.lvl <= 2 then
    	self:LowQ(ltarg)
    end
...
function Riven:LowQ(target)
	local ltarg = _G.SDK.TargetSelector:GetTarget(850)
		if myHero.levelData.lvl <= 2 then
			if myHero.pos:DistanceTo(ltarg.pos) < 390 then
            	if self.Menu.Combo.UseQ:Value() and self:CanCast(_Q) and myHero.attackData.state == STATE_WINDDOWN then
                	local pred=ltarg:GetPrediction(Q.speed,.25 + Game.Latency()/1000)
                	Control.CastSpell(HK_Q,pred)
                	Control.Attack(ltarg)
...
function Riven:CastQ(target)
    ...
    if qtarg then
        ...
        if myHero.pos:DistanceTo(qtarg.pos) < 275 and self:HasBuff(myHero, "rivenwindslashready") then    --myHero.range
            if self.Menu.Combo.UseQ:Value() and self:CanCast(_Q) and myHero.attackData.state == STATE_WINDDOWN then
        	...
        	if myHero.pos:DistanceTo(qtarg.pos) < 200 and not self:HasBuff(myHero, "rivenwindslashready") then    --Q without buff less range
            	if self.Menu.Combo.UseQ:Value() and self:CanCast(_Q) and myHero.attackData.state == STATE_WINDDOWN then
                	local pred=qtarg:GetPrediction(Q.speed,.25 + Game.Latency()/1000)
                	Control.CastSpell(HK_Q,pred)
                	Control.Attack(qtarg)
