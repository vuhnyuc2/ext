...
function Malzahar:LoadMenu()
	...
	Tocsin.Pred:MenuElement({id = "Chance", name = "Prediction Hitchance Q", value = 0.200, min = 0.05, max = 1, step = 0.025})
...
function Malzahar:Misc()
	...
	if not target then return end
		if Tocsin.Misc.Eks:Value() and Ready(_E) then
			...
			if  Edmg > target.health * 1.2 + target.shieldAP then
				self:CastE(target)
			end
		end
        ...
        local QEdmg = Edmg + Qdmg
            if QEdmg > target.health * 1.3 + target.shieldAP then
                self:CastE(target)
